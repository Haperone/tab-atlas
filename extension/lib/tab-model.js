...
export function tabsSignature(list) {
  return (list || []).map(tab => JSON.stringify([
    tab.id,
    tab.title || '',
    tab.url || '',
    !!tab.pinned,
...
const DASHBOARD_UPDATE_FIELDS = new Set([
  'title', 'url', 'pinned', 'audible', 'mutedInfo', 'discarded', 'groupId',
]);

export function tabUpdateAffectsDashboard(changeInfo) {
  return Object.keys(changeInfo || {}).some(key => DASHBOARD_UPDATE_FIELDS.has(key));
}
...
export function favIcon(pageUrl, size = 16, runtime = globalThis.chrome?.runtime) {
  try {
    if (runtime?.getURL) {
...
export function smartTitle(title, url) {
  ...
  if ((hostname === 'www.reddit.com' || hostname === 'reddit.com' || hostname === 'old.reddit.com') && pathname.includes('/comments/')) {
    ...
    if (subredditIndex !== -1 && parts[subredditIndex + 1] && titleIsUrl) return `r/${parts[subredditIndex + 1]} post`;
  }
  return title || url;
}

export function isLandingPageUrl(url, patterns) {
  try {
    const parsed = new URL(url);
